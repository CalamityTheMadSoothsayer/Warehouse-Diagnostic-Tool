...
Generates a SQL script to delete InventoryCasesQAStatuses records tied to
...
DESCRIPTION = (
    "Generates a SQL DELETE script to remove InventoryCasesQAStatuses records "
...
# Selects stale InventoryIds that also have a linked QA status row.
# These must be deleted alongside (or before) the InventoryCase is moved to
# LVADJ — orphaned QA statuses can block future inventory processing.
SQL_IDS = """
...
def run() -> QueryResult:
    result = QueryResult()
    result.add_message("info", f"[{TITLE}] Generating fix script for duplicate QA Statuses...")
    ...
    if not ids:
        # No duplicate cases have linked QA status records — nothing to clean up
        result.status   = "ok"
        result.headline = "No linked QA Statuses found — no script needed."
    ...
    lines = [
        ...
        "BEGIN TRAN t1",
        "",
        "DELETE FROM InventoryCasesQAStatuses",
    ...
    result.headline = f"Script generated for {len(ids)} QA Status record(s). Copy and run in SSMS."
    result.data     = lines
